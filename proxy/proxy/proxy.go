...
import (
	"fmt"
	. "goproxy/datastruct"
	"goproxy/internal/cfg"
	"goproxy/internal/modfetch"
	"goproxy/internal/modfetch/codehost"
	"goproxy/internal/modload"
	"goproxy/internal/module"
	"log"
	"net/http"
	"os"
	"path"
	...
	"github.com/soniashiss/tinylib/requests"
)
...
func setupEnv(basedir string) {
	...
	modfetch.PkgMod = filepath.Join(basedir, "proxy", "mod")
		...
		case ".info", ".mod", ".zip":
			{
				...
				if needProxy {
					data, err := requests.SendGET(replceUrl(r.URL.Path, proxyInfo), "")
					if err != nil {
...
func replceUrl(url string, proxyInfo *ProxyInfo) string {
	proxyAddress := proxyInfo.RemoteProxyAddress
	if "" != proxyAddress {
		if !strings.HasSuffix(proxyInfo.RemoteProxyAddress, "/") {
			proxyAddress = proxyAddress + "/"
		}
		url = proxyAddress + url
	}
	return url
}
